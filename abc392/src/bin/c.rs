...
fn main() {
   ...
   let P = P.into_iter().map(|p| Person(p.0 - 1)).collect::<Vec<_>>();
   let Q: Vec<Bib> = Q;
   let players = Q.into_iter().enumerate().map(|(i, bib)| Player {
      number: Person(i),
      bib,
   });

   let player_to_bib: HashMap<Person, Bib> = players.clone().map(|p| (p.number, p.bib)).collect();
   let ans = players
      .sorted_unstable_by(|p1, p2| p1.bib.cmp(&p2.bib))
      .map(|p| player_to_bib.get(&P[p.number.0]).unwrap().0)
      .join(" ");
   println!("{}", ans);
}
